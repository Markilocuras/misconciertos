...
- `_authenticated/route.tsx` — layout route; `beforeLoad` checks `supabase.auth.getUser()` client-side and redirects to `/auth` if unauthenticated (`ssr: false`). Children: `admin.stats.tsx` (clics en "Comprar entradas" + suscripciones por mail), `admin.revision.tsx` (la cola de dudosos), `admin.comentarios.tsx` (moderación) y `perfil.tsx` (saved concerts + own comments).
  ...
  Las suscripciones del panel son las **tres listas de mail** (`concert_digest_subscriptions`, `artist_alerts`, `show_email_reminders`), no el push: un mail identifica una persona y un endpoint identifica un navegador, así que sumarlos daría un número que no es ni gente ni dispositivos. Las cuentas viven en `src/lib/suscripciones.ts` —puras, con test— y cada grupo viaja con sus direcciones: la fila se abre y las muestra, porque saber **cuántos** casi nunca es la pregunta completa. Van en el mismo payload y no en una consulta por fila abierta: son decenas, y el techo diario de Resend las deja alrededor de 90. Si alguna vez son miles, eso pasa a ser un server fn aparte por grupo. Del resumen diario se listan sólo las confirmadas —la dirección de una cuenta sin verificar ni siquiera sale de `auth.users`, que es la misma razón por la que no se le escribe—. Los avisos por artista se agrupan por `slugify` y no por el texto crudo, porque así los agrupa el envío: "Dillom" y "DILLOM" son una sola audiencia que recibe un solo mail.
...
- `public.artist_comments` — publicly readable, INSERT restricted to `auth.uid() = user_id`. FK points at `profiles`, not `auth.users`, so the embed above works. **Nadie tiene DELETE** —no hay política ni grant, tampoco para el autor— así que moderar va por `/admin/comentarios` con el cliente service-role (`src/lib/comments.functions.ts`). Eliminar marca `deleted_at`/`deleted_by` en vez de borrar la fila: un clic de más se deshace, y queda a la vista quién viene subiendo lo que hay que sacar.

  Ojo con la parte que no es obvia: **lo que esconde el comentario es la política de SELECT** (`deleted_at is null`, desde `20260920190000_moderar_comentarios.sql`), no el panel. Es la trampa del borrado blando y vale para cualquier tabla pública que lo use acá — filtrando sólo en el cliente, la fila sigue saliendo por PostgREST con la anon key, que es pública y viaja en el bundle, así que "borrado" sería nada más que invisible en la ficha. El panel ve los eliminados justamente porque el service-role no pasa por RLS.
