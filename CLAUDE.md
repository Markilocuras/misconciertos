...
Live Pass es la única que publica coordenadas propias (en el JSON-LD de cada evento) y `toRow` las prefiere por sobre `VENUE_COORDS`: no dependen de que alguien haya cargado el lugar a mano. También es la única que hay que filtrar por provincia, porque vende en todo el país (`isBuenosAiresRegion`).

Cada fuente reporta `found`/`scraped`/`upserted`/`discarded`/`skipped`, más `discardedBy` (por qué se cayó cada fila) y `unknownVenues` (los venues que faltan cargar en `VENUE_COORDS`).
