...
// Baja de los avisos por artista. Gemelo de unsubscribe-digest: mismo
// mecanismo, distinta tabla. Son dos suscripciones distintas y la baja de una
// no toca la otra — quien se anoto a un artista puntual no pidio dejar de
// recibir el resumen, ni al reves.
//
...
export const Route = createFileRoute("/api/public/hooks/unsubscribe-alert")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const { data, error } = await supabaseAdmin.rpc("unsubscribe_artist_alert", { token });
          if (error) {
            console.error("[unsubscribe-alert] rpc failed", error);
